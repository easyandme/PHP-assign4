...
function get_technicians() {
	global $db;
	$query = 'SELECT * FROM assign3_technicians
			  ORDER BY techID';
...
function delete_technician($tech_ID) {
	global $db;
	$query = "DELETE FROM assign3_technicians
...
function add_technician($first_name, $last_name, $email, $phone, $password) {
	global $db;
	$query = "INSERT INTO assign3_technicians
...
function get_technicians_with_open_incidents_count() {
	...
	$query = 'SELECT techID, firstName, lastName, 
				(SELECT COUNT(*) FROM assign3_incidents
				 WHERE (dateClosed IS NULL AND assign3_technicians.techID = assign3_incidents.techID)) AS incidentCount
			 FROM assign3_technicians
