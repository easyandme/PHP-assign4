...
function add_incident($customerID, $product_code, $techID, $open_date, $title, $description) {
	...
	$query = "INSERT INTO assign3_incidents
...
function get_unassigned_incidents() {
	global $db;
	$query = 'SELECT * FROM assign3_incidents
				INNER JOIN assign3_customers
		    			ON assign3_incidents.customerID = assign3_customers.customerID
				INNER JOIN assign3_products
						ON assign3_incidents.productCode = assign3_products.productCode
...
function get_assigned_incidents() {
	...
	$query = 'SELECT * FROM assign3_incidents
				...
				INNER JOIN assign3_products
						ON assign3_incidents.productCode = assign3_products.productCode
			  WHERE assign3_incidents.techID IS NOT NULL';
	try {
		$statement = $db->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
...
function update_incident($incidentID, $customerID, $product_code, $techID, $open_date, $close_date, $title, $description) {
	global $db;
	$query = 'UPDATE assign3_incidents
